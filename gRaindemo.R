...
### chunk number 4: 
querygrain(pn,nodes=c("lung","bronc"))


### chunk number 5: 
pn2  <- setFinding(pn,nodes=c("asia","dysp"),states=c("yes","yes"))


### chunk number 6: 
querygrain(pn2,nodes=c("lung","bronc"))


### chunk number 7: 
querygrain(pn2,nodes=c("lung","bronc"), type="joint")


### chunk number 8: 
#pnc <- compile.grain(pn)
pnc<-get("compile.cpt-grain","package:gRain")(pn)
...
### chunk number 10: 
querygrain(pnc,nodes=c("lung","bronc"))
querygrain(pnc,nodes=c("lung","bronc"),type="joint")
querygrain(pnc,nodes=c("lung","bronc"),type="conditional")


### chunk number 11: 
pnc2  <- setFinding(pnc,nodes=c("asia","dysp"),states=c("yes","yes"))
#pnc2  <- setFinding(pnc,evlist=list(c("asia","yes"),c("dysp", "yes")))
...
### chunk number 14: 
querygrain(pnc2,nodes=c("lung","bronc"))
querygrain(pnc2,nodes=c("lung","bronc"), type="joint")
querygrain(pnc2,nodes=c("lung","bronc"), type="conditional")


### chunk number 15: 
pnc2  <- setFinding(pnc,nodes=c("asia"),states=c("yes"),propagate=FALSE)
##Finding(pnc2)
pnc2  <- setFinding(pnc2,nodes=c("dysp"),states=c("yes"),propagate=FALSE)
##Finding(pnc2)
pnc2 <- propagate(pnc2)
getFinding(pnc2)
...
### chunk number 16: 
pnc3 <- retractFinding(pnc2, nodes="asia")
getFinding(pnc3)
...
### chunk number 20: 
pnc2 <- get("compile.cpt-grain","package:gRain")(pn, root=c("lung", "bronc", "tub"), propagate=TRUE)


### chunk number 21: 
system.time({for (i in 1:10) querygrain(pnc,nodes=c("lung","bronc", "tub"),type="joint")})
system.time({for (i in 1:10) querygrain(pnc2,nodes=c("lung","bronc", "tub"),type="joint")})
...
### chunk number 33: 
pnx <- grain(dag, gmData=as.gmData(chestSim))
pnx <- compile.grain(pnx, propagate=TRUE)
...
### chunk number 35: 
pny <- grain(ug, as.gmData(chestSim))
pny <- compile.grain(pny, propagate=TRUE)
