...
spadek_gradientu <- function(func, pochodna, start, step=0.05, tol=1e-8) {
  pt1 <- start                               # start
  grdnt <- pochodna(pt1)                     # pochodna funkcji 
  pt2 <- pt1[1] - step*grdnt[1]              # odejmujemy pochodną razy step od wejściowego parametru
  while (abs(func(pt1)-func(pt2)) > tol) {   # przy minimum małe wartości pochodnej, jeśli mniejsze od tol przerwij pętle
    ...
    pt2 <- pt1[1] - step*grdnt[1]            # aby "stoczyć się" do minimum
...
spadek_gradientu <- function(func, gradient, start, step=0.05, tol=1e-8) {
  pt1 <- start                               # start
...
################################################################################
# Funkcja kosztów - kwadraty błędów
cost <- function(X, y, theta) {
  sum( (X %*% theta - y)^2 ) / (2*length(y))
}

alpha <- 0.01                                # współczynnik uczenia alfa
...
for (i in 1:num_iters) {                     # spadek gradientu dla sumy kwadratów różnic po osi y
  ...
  theta <- theta - alpha * delta             # zwiększamy lub zmniejszamy theta 
